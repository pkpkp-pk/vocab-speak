...
//
// Protocol:
//   in : { type: "prewarm" } — start the model download/load early
//        { type: "analyze", audio: Float32Array (16 kHz mono, transferred),
//          transcript: string | null }
//   out: { type: "progress", stage, progress (0..1), detail }
//        { type: "done", words: [{ word, score, start, end }], decodedTranscript?, truncated }
//        { type: "error", message }
...
async function runInference(audio, transcript) {
  ...
  // Without a recognizer transcript (Brave/Firefox/Safari), decode one locally
  // and score against it — self-consistency instead of an independent reference.
...
self.onmessage = async (e) => {
  const msg = e.data;
  if (msg.type === "prewarm") {
    loadBundle().catch(() => {}); // a failure here surfaces on analyze instead
    return;
  }
